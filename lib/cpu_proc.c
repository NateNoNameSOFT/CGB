...
void cpu_set_flags(cpu_context *ctx, char z, char n, char h, char c){
    ...
    if(h != -1){
        BIT_SET(ctx->regs.f, 5, z);
    }
    if(c != -1){
        BIT_SET(ctx->regs.f, 4, z);
    }
}
...
static void proc_ld(cpu_context *ctx){
    if(ctx->dest_is_mem){
        ...
        if(ctx->cur_inst->reg_2 >= RT_AF){      // if it is a 16-bit reg
    ...
    if(ctx->cur_inst->mode == AM_HL_SPR){
        ...
        u8 cflag = (cpu_read_reg(ctx->cur_inst->reg_2) & 0xF) + (ctx->fetched_data & 0xFF) >= 0x10;
...
static void goto_addr(cpu_context *ctx, u16 addr, bool pushpc){
    if(check_cond(ctx)){
        if(pushpc){
            emu_cycles(2);              //2 because it is a 16-bit addr
